...
const URL: string = 'https://www.filmweb.pl/films/search?orderBy=popularity&descending=true';
...
(async () => {
  try {
    ...
    fs.writeFile(`${dir}result.json`, JSON.stringify(mergedMovies), (err: any) => {
      if (err) throw err;
...
async function grabMovies(page: pptr.Page) {
  ...
  await asyncForEach(moviesEl, async (movieEl) => {
    ...
    if (!originalTitleEl) {
      ...
      const originalTitleElProp = await originalTitleEl.getProperty('innerText');
